...
#### DNSのネームサーバー設定（前提）
OpenShiftクラスタの構築を始める前に、Base domainとして使うドメインのDNSをOCIで管理できる状態にしておく必要があります。
今回は `mosuke5.com` を使う予定だったので、事前にドメインのレジストラ側でネームサーバーをOCIのものに切り替えておきました。

![ネームサーバー設定の確認](/image/openshift-on-oci-dns-name-server.png)

OCIでDNSを管理しておくと、Terraformによるクラスタ構築時にDNSレコードの登録まで自動で行ってくれます。もちろん、別のDNSサービスで管理していても構築自体は可能ですが、その場合はレコードの登録を自分で行う必要があります。
...
- **Base domain**: 今回は `mosuke5.com` を使用しました（1.3で説明したとおり、OCIのネームサーバーに切り替え済みです）。
